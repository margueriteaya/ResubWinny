...
(function () {
  ...
  window.createSplineLayer = function createSplineLayer(gl, canvas) {
    ...
    const bgProg = link(
      ...
      `#version 300 es
       ...
       void main() {
         ...
         vec3 background = mix(uColorStart, uColorEnd, g);
         oColor = vec4(background * uBackgroundAlpha, uBackgroundAlpha);
       }`
    );
    ...
    const bgU = {
      ...
      backgroundAlpha: uloc(gl, bgProg, 'uBackgroundAlpha'),
    };
    ...
    const waveU = {
      ...
      verticalOffset: uloc(gl, waveProg, 'verticalOffset'),
    ...
    function render(timeSec) {
      ...
      gl.uniform1f(bgU.backgroundAlpha, window.RESUBWINNY_XMB_BACKGROUND_ALPHA ?? 1.0);
      ...
      gl.uniform1f(waveU.verticalOffset, window.RESUBWINNY_XMB_VERTICAL_OFFSET ?? 0.0);
