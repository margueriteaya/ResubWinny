...
export type AppSettings = { uiFont: 'system' | 'cjk' | 'arib'; captionFont: 'arib' | 'system'; defaultFormat: 'ASS' | 'TTML' | 'JSON' | 'Raw Data'; defaultTimeline: 'Auto (Gap Merge + Overlap Resolve)' | 'Strict original timestamps'; locale: string; theme: 'system' | 'light' | 'dark'; workspaceLayout: WorkspaceLayoutSettings }
...
export type PreviewOverlaySyncResult = { action: 'applied' | 'cleared' | 'unchanged' | 'awaiting-player-time'; mediaTimeMs: number | null; projectTimeMs: number | null; snapshot: CaptionRenderSnapshot | null }
