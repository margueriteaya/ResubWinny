...
<style>
  ...
  .slider-thumb{position:absolute;z-index:2;top:4px;left:calc(var(--slider-progress) - var(--slider-thumb-offset));display:block;width:16px;height:16px;overflow:hidden;border:.5px solid var(--rw-glass-border);border-radius:50%;background:transparent;box-shadow:0 1px 3px rgba(0,0,0,.22),inset 0 1px rgba(255,255,255,.82),inset 0 -1px var(--rw-glass-dark-edge);pointer-events:none;transition:left 80ms linear,box-shadow 120ms ease}
