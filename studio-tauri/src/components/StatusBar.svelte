...
<footer class="status-bar">
  ...
  {#if isExporting}
    <div class="task-progress"><span>{previewIndexing ? t("task.indexingTimeline") : isPaused ? t("task.pausedStatus") : t("task.processing")} ({progress.toFixed(1)}%)</span><i><b style={`width:${progress}%`}></b></i></div>
