...
<label class="mac-checkbox" class:disabled data-tooltip={label || undefined}>
  ...
  <span class="checkbox-box" class:mixed aria-hidden="true"><svg class="checkmark" class:visible={checked && !mixed} viewBox="0 0 10 10"><path d="M1 5.15 3.65 8 9 1.55" /></svg></span>
...
<style>
  ...
  .checkbox-box{display:grid;place-items:center;width:16px;height:16px;flex:0 0 16px;border:1px solid color-mix(in srgb,var(--rw-text) 28%,transparent);border-radius:5px;background:color-mix(in srgb,var(--rw-content) 86%,var(--rw-text) 4%);box-shadow:inset 0 .5px rgba(255,255,255,.75),0 .5px 1px rgba(0,0,0,.08);transition:background-color var(--rw-motion-responsive) var(--rw-ease-out),border-color var(--rw-motion-responsive) var(--rw-ease-out),box-shadow var(--rw-motion-responsive) var(--rw-ease-out)}
  ...
  .checkmark{display:block;width:10px;height:10px;overflow:visible;filter:none;opacity:0;transform:scale(.68);transition:opacity var(--rw-motion-fast) var(--rw-ease-out),transform var(--rw-motion-fluid) var(--rw-ease-spring)}.checkmark.visible{opacity:1;transform:none}.checkmark path{fill:none;stroke:#fff;stroke-width:1.9;stroke-linecap:round;stroke-linejoin:round;vector-effect:non-scaling-stroke}
  .checkbox-label{line-height:16px}
  .mac-checkbox.disabled{cursor:not-allowed;opacity:.62}
  @media(prefers-reduced-motion:reduce){.checkbox-box,.checkmark{transition:none}}
</style>
