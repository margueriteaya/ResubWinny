<script lang="ts">
  ...
  function handleKeydown(event: KeyboardEvent, index: number) {
    ...
    requestAnimationFrame(() => keyboardNavigation = false);
  }
</script>

<div class:toolbar={size === "toolbar"} class:icon-only={iconOnly} class:keyboard-navigation={keyboardNavigation} class:liquid-control={size !== "toolbar"} class="mac-segmented" role="radiogroup" aria-label={ariaLabel} style={`--segment-count:${Math.max(1, options.length)};--segment-offset:${selectedIndex * 100}%`}>
