...
<style>
  ...
  .switch-track{position:relative;display:block;width:38px;height:22px;flex:0 0 38px;border:1px solid color-mix(in srgb,var(--rw-text) 16%,transparent);border-radius:11px;background:color-mix(in srgb,var(--rw-text) 18%,transparent);box-shadow:inset 0 .5px rgba(0,0,0,.12),inset 0 1px rgba(255,255,255,.54);transition:background-color var(--rw-motion-responsive) var(--rw-ease-out),border-color var(--rw-motion-responsive) var(--rw-ease-out),box-shadow var(--rw-motion-responsive) var(--rw-ease-out)}
  .switch-thumb{position:absolute;z-index:2;top:1px;left:1px;width:18px;height:18px;overflow:hidden;border:.5px solid var(--rw-glass-border);border-radius:50%;background:transparent;box-shadow:0 1px 3px rgba(0,0,0,.25),inset 0 .5px rgba(255,255,255,.92),inset 0 -1px var(--rw-glass-dark-edge);transition:transform var(--rw-motion-fluid) var(--rw-ease-spring)}
