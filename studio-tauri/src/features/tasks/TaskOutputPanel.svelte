...
<section class="output-panel">
  ...
  {#if assessmentRows.length}<section class="assessment" aria-live="polite"><b>输出评估</b>{#each assessmentRows as row}<div class="assessment-row"><strong>{row.format}</strong>{#if row.result?.approximated.length}<span class="approx">△ {row.result.approximated.join('、')} 将兼容近似</span>{/if}{#if row.result?.conditional.length}<span class="conditional">◇ 部分内容尚未确认</span>{/if}{#if row.result?.conflicts.length}<span class="conflict">× 存在无法满足的保留要求</span>{/if}{#if row.result?.dropped.length}<span class="dropped">已关闭：{row.result.dropped.join('、')}</span>{/if}</div>{/each}</section>{/if}
...
<style>
  ...
  .assessment{display:grid;gap:6px;margin-top:14px;padding:10px 11px;border:1px solid var(--rw-border);border-radius:7px;background:var(--rw-content);font-size:11px}.assessment>b{font-size:12px}.assessment-row{display:grid;grid-template-columns:72px minmax(0,1fr);gap:7px;align-items:start}.assessment-row strong{color:var(--rw-text)}.assessment-row span{line-height:15px}.approx{color:var(--rw-warning)}.conditional{color:var(--rw-text-secondary)}.conflict{color:#c24848}.dropped{color:var(--rw-muted)}
