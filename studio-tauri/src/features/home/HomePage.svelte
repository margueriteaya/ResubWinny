<script lang="ts">
  ...
  const capabilityLegend = [
    ...
    { tone: "attention", label: "home.capability.attention", detail: "home.capability.attentionDetail" },
  ] as const;
            ...
            <div class="format-group-buttons">
              {#each group.formats as format}
                ...
                <button type="button" class:selected={settings.exportPreferences.formats.includes(format)} class:explained={explainedFormat === format} aria-pressed={settings.exportPreferences.formats.includes(format)} aria-label={`${format} ${visual.extension} · ${formatSummary}`} title={formatSummary} onclick={() => toggleFormat(format)}>
                  ...
                  <span class="format-name"><b>{format}</b><small>{visual.extension}</small><span class="format-capability-meter" aria-hidden="true">{#each formatCapabilities(format) as capability}<i class={`meter-${capability.level}`}></i>{/each}</span></span>
                </button>
              {/each}
            </div>
          </div>
