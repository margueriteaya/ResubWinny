<script lang="ts">
  ...
  const capabilityLegend: CapabilityLevel[] = ["preserved", "approximated", "conditional", "unsupported"];
  ...
  <section class="home-output-preferences" aria-label={t("home.outputPreferences")}>
    ...
    <div class="preference-group preservation-preference">
      ...
      <section class="capability-legend" aria-label={t("home.capability.legend")}>
        ...
        {#each capabilityLegend as level}
          <div class={`legend-row level-${level}`}>
            <span class="status-swatch" aria-hidden="true"></span>
            <span><b>{levelLabel(level)}</b><small>{levelDetail(level)}</small></span>
          </div>
        {/each}
      </section>
    </div>
