<script lang="ts">
  ...
  import { featureVisuals } from "../tasks/feature-visuals";
    ...
    <div class="preference-group format-preference">
      ...
      <div class="capability-chips" role="toolbar" aria-label={t("home.capability.features")}>
        {#each explainedCapabilities as capability (capability.feature)}
          {@const FeatureIcon = featureVisuals[capability.feature].icon}
          <button type="button" class={`level-${capability.level}`} aria-pressed={activeCapabilityFeature === capability.feature} title={levelLabel(capability.level)} onclick={() => activeCapabilityFeature = capability.feature}><span class="feature-icon" aria-hidden="true"><svelte:component this={FeatureIcon} size={14} stroke={1.8} /></span><i>{capabilityIcon[capability.level]}</i>{t(`feature.${capability.feature}`)}</button>
        {/each}
      </div>
      {#if activeCapability}
        {@const ActiveFeatureIcon = featureVisuals[activeCapability.feature].icon}
        <div class={`capability-detail level-${activeCapability.level}`} aria-live="polite"><b><span class="detail-feature-icon" aria-hidden="true"><svelte:component this={ActiveFeatureIcon} size={14} stroke={1.8} /></span><i>{capabilityIcon[activeCapability.level]}</i>{t(`feature.${activeCapability.feature}`)} · {levelLabel(activeCapability.level)}</b><p>{levelDetail(activeCapability.level)}</p></div>
      {/if}
    </div>
    <div class="preference-group preservation-preference">
      ...
      <div class="preservation-picker">
        {#each preservationKeys as key}
          {@const FeatureIcon = featureVisuals[key].icon}
          <label><input type="checkbox" checked={settings.exportPreferences.preservation[key]} onchange={() => togglePreservation(key)} /><span class="preservation-feature-icon" aria-hidden="true"><svelte:component this={FeatureIcon} size={15} stroke={1.8} /></span><span>{t(`feature.${key}`)}</span></label>
        {/each}
      </div>
    </div>
  </section>
