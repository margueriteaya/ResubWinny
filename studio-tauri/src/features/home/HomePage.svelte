  ...
  <div class="approved-home-grid">
    <div class="approved-home-primary">
      ...
      <section class="approved-section recent-section">
        <header class="approved-section-head"><h2>{t("home.recent")}</h2><button class="approved-icon" data-tooltip={t("home.viewAll")} aria-label={t("home.viewAll")} onclick={onOpenTask}><Ellipsis size={16} /></button></header>
        {#if history.length}
          <div class="recent-table-wrap">
            <table class="recent-table">
              ...
              <tbody>
                {#each history.slice(0, 5) as item, index (item.path)}
                  <tr class:selected={index === 0}>
                    <td><button class="recent-file" onclick={() => onOpenHistory(item)} disabled={isInspecting}><span class="recent-file-icon"><FileVideo2 size={16} /><small>{item.container}</small></span><span><b>{item.name}</b><small>{bytes(item.size)} · {item.container}</small></span></button></td>
