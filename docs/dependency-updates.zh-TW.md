...
| 類別 | 範例 | 固定與更新規則 |
| --- | --- | --- |
| 隨附原始碼 | libaribcaption、libaribtlv、Zlib | 在 `third_party/versions.json` 中固定上游標籤、完整提交和確定性的原始碼快照雜湊；審查原始碼差異及授權條款，隨後用 `scripts/prepare-vendored-source.ps1` 移除巢狀 Git 中繼資料。 |
...
- **libaribtlv 與 Zlib：** 建置 vendored 靜態橋接，不得回退到系統 Zlib；執行 native TLV feature 的通訊協定回歸及嚴格 TTML/EXI raw-evidence 測試。審查上游 C ABI、callback 所有權、resource 與時間語義；缺少的 MPU/MMTP/NTP 資訊必須保持缺少，不能以零值或猜測補齊。
