...
再配布可能な放送録画が存在しないことは、リリースの不備ではありません。実際の録画は、法的理由により非公開の検証環境でのみ保持およびテストされます。公開 CI は構築されたプロトコルフィクスチャを使用し、録画のバイト列、その録画から派生した字幕、スクリーンショットを公開せずにテスト結果を公開します。リリースノートでスキップしたゲートを明記し、公開の合成チェックに合格していれば、非公開コーパスゲートをスキップしてリリースを進めることができます。
...
コード署名は、この階層の前提条件ではありません。バンドルされた libmpv ビルドに対応するソースとライセンスへの準拠は引き続き必須であり、ビルドを Alpha または未署名と表示することで免除することはできません。
