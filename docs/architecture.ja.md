...
表示 plane 補正（2026-07-25、2026-09-02 改訂）：root の `<tt>` が有効な pixel display extent を宣言した場合、それを B62/ARIB-TTML の source coordinate space とします。これが無い場合は logical 2K convention を既定とし、完全な pixel `origin`/`extent` geometry が少なくとも一方の軸で 2K を超えた時だけ、各 coordinate/size component を収める最小の標準 3840×2160 または 7680×4320 plane を選びます。region extent は display edge で clip され得る layout capacity なので、`origin + extent` で 4K document を 8K に昇格させません。ただし最大標準 plane を越える総境界は推定を拒否します。region geometry は横・縦ごとに scale し、pixel の font size、line height、letter spacing、直接 outline width は有界な共通 scale を用います。現在の native backend の `1920×1080` plane は中間 logical texture であり、正しさを定義する目標解像度ではありません。正しさは、source layout の比率が letterbox/pillarbox を除いた実際の video-content viewport に対応し、window size、DPI、fullscreen の変化を通じて維持されることで評価します。曖昧または不正な input を 4K とみなすことはありません。raw PES/MMTP evidence は変更しません。
...
ARIB-TTML background/outline 修正（2026-09-02）：caption model は有効な `backgroundColor` が region、block、inline のどこに由来するかを保持します。native preview は region scope の色だけを region 全体に描画し、block/inline の色は実測した text line または glyph cell に合わせます。同時に有効な geometry と colour が同一の region background は一度だけ合成します。scope を持たない旧 archive は region の layout capacity を字幕 box と誤認せず、compact text background として扱います。明示的な `textOutline` width `0px` は outline なしを意味し、最小 1 px に引き上げません。継承した inline colour に parent opacity を適用するのは一度だけです。
