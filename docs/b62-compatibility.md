...
| 声明的或有证据支持的显示平面 | 有效的根 `tts:extent` 具有权威性并定义源坐标空间。若无该值，逻辑 2K 仍为默认值；仅当完整像素 `origin`/`extent` 几何在至少一个轴上超过逻辑 2K 且仍处于相应平面范围内时，解析器才推断规范的 3840×2160 或 7680×4320。区域 origin/extent 使用独立的水平/垂直比例；像素字体大小、行高、字母间距和直接轮廓宽度使用有界的统一比例。当前 `1920×1080` RGBA 平面只是后端中间纹理；验收依据是源布局按比例映射到排除黑边后的视频内容 viewport，并在窗口大小、DPI 和全屏状态变化时保持观看者相对区域；绝不猜测有歧义的输入 |
