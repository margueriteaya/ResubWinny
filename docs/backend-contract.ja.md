...
バックエンドサーフェスは、制限された安定したアプリケーションコントラクトです。その間、
...
TLV アーカイブエクスポートには、制限された `asset_evidence` と
`resource_evidence` レコード。各 `resource_evidence` レコードには、
ロスレス Base64 ペイロード、フォーマット検証、および正確な
`packet_id + mpu_sequence_number + subsample_number` レコードキーが使用されます
`subt://` リファレンスと一致します。アーカイブ時のプレビューリーダーは最大 64 を保持します
...
レンダリングされたキャプションテキストとして主張されていません。
...
アソシエーションは `same-mpu-evidence` で、その raw リソースレコードを指します。
...
フォントテーブルのカウントが存在する場合、それは構造メタデータのみです。小さい、
構造的に完全な PNG リソースには、キャップ付きの `data:` プレビューが含まれる場合もあります
将来のネイティブプレビューサーフェスの値。バックエンドはまだデコードできません
...
固定されたローカルベースラインとレビュールールは `docs/visual-reference.md` にあります。
このプロファイルの B24 部分はデコーダでサポートされています。現在のネイティブ TTML パス
バンドルされたフォント、ソースの前景/背景 RGBA、スパンスタイルの実行を使用します。
...
シャドウまたは固定ブラックボックス。
`captionOverlayModes` は、構造化されたバックエンドルート機能の配列です。
...
- `get_preview_capabilities` は、各ルートを `{ id、available、experimental、
  利用できない理由コード }`。これはプレゼンテーションコントラクトのみです: WebView
  キャプションビットマップを送信できません。`render_preview_at`と
  `sync_preview_overlay` は、境界のあるネイティブキャプションプレーンを内部に構成します。
  バックエンドを作成し、それを libmpv に適用します。Windows 以外のビルドレポート
  `preview.platform_not_implemented`;ネイティブプレビュールートを意味するものではありません。
- `sync_preview_overlay` は、`mediaTimeMs` と `projectTimeMs` の両方をレポートします。`projectTimeMs` を使用してキャプションをクエリします。デフォルトのマッピングは ID ですが、PTS 修復、プログラム境界、およびユーザーオフセットは、WebView に 2 番目のクロックを教えるのではなく、バックエンドマッピングを更新する必要があります。
- `trackId` は、検出されたすべての MPEG-TS の検証済み PID セレクターとして渡されます。
  B24 または M2TS データトラック。B24 の場合、選択された PID は論理
  ...
  ディスカバリ `caption_pids`、コンポーネントタグ、PAT/PMT サービス ID、SDT サービス
  ...
  メタデータは製品契約の外に残ります。キュースーパーバイザが一時停止を所有します
...
`superimpose_pids`。コンポーネントタグ `0x30..0x37` および `0x38..0x3f` は、
...
独立した。PMT 記述子がプライベートストリームを分類しない場合、プライベートストリームはそのままになります。
...
M2TS ルートは、このドキュメントクロックを 30 ビットの到着タイムスタンプから取得します。
